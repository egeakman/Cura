            ...
            if parts:
                ...
                if self._max_travel > 0:
                    ...
                    self._distance = math.sqrt(
                        self._abs_delta[0] ** 2 + self._abs_delta[1] ** 2 + self._abs_delta[2] ** 2
                    )
                    if self._distance == 0:
                    ...
                    if buf.previous_nominal_feedrate > 0.0001:
                        ...
                        if abs(current_feedrate[2] - buf.previous_feedrate[2]) > MACHINE_MAX_JERK_Z:
                            vmax_junction_factor = min(
                                ...
                                (MACHINE_MAX_JERK_Z / abs(current_feedrate[2] - buf.previous_feedrate[2])),
                            )
                        if abs(current_feedrate[3] - buf.previous_feedrate[3]) > MACHINE_MAX_JERK_E:
                            vmax_junction_factor = min(
                                vmax_junction_factor,
                                (MACHINE_MAX_JERK_E / abs(current_feedrate[3] - buf.previous_feedrate[3])),
                            )
                        vmax_junction = min(
                            buf.previous_nominal_feedrate, vmax_junction * vmax_junction_factor
                    ...
                    self.calculate_trapezoid(
                        self._entry_speed / self._nominal_feedrate, safe_speed / self._nominal_feedrate
                    )
...
class CommandBuffer:
    ...
    def process(self) -> None:
        ...
        if current is not None and current.estimated_exec_time >= 0:
            current.calculate_trapezoid(
                current._entry_speed / current._nominal_feedrate, MINIMUM_PLANNER_SPEED / current._nominal_feedrate
            )
        ...
        for cmd in self._all_commands:
            ...
            cmd.estimated_exec_time += calc_acceleration_time_from_distance(
                cmd._final_feedrate, (cmd._distance - cmd._decelerate_after), cmd._acceleration
            )
                ...
                if total_frame_time <= self._detection_time_frame and cmd_count > self._code_count_limit:
                    ...
                    if need_to_append:
                        self._bad_frame_ranges.append(
                            {"start_line": cmd0_idx, "end_line": idx, "cmd_count": cmd_count, "time": total_frame_time}
                        )

    def reverse_pass_kernel(
        self, previous: Command | None, current: Command | None, next: Command | None
    ) -> None:
        ...
        if current._entry_speed != current._max_entry_speed:
            ...
            if not current._nominal_length and current._max_entry_speed > next._max_entry_speed:
                current._entry_speed = min(
                    ...
                    calc_max_allowable_speed(-current._acceleration, next._entry_speed, current._distance),
                )
            else:
    ...
    def forward_pass_kernel(
        ...
        # to recheck.
        if not previous._nominal_length and previous._entry_speed < current._entry_speed:
            entry_speed = min(
                current._entry_speed,
                calc_max_allowable_speed(-previous._acceleration, previous._entry_speed, previous._distance),
            )
