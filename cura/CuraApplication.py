...
class CuraApplication(QtApplication):
    ...
    def startSplashWindowPhase(self) -> None:
        ...
        if not self.getIsHeadLess():
            try:
                self.setWindowIcon(QIcon(Resources.getPath(Resources.Images, "cura-icon_wip.png" if ApplicationMetadata.IsAlternateVersion else "cura-icon.png")))
    ...
    def setDefaultPath(self, key, default_path):
        self.getPreferences().setValue(f"local_file/{key}", QUrl(default_path).toLocalFile())
    ...
    def _loadPlugins(self) -> None:
        ...
        lib_suffixes = {"", "64", "32", "x32"} if Platform.isLinux() else {""} # A few common ones on different distributions.
    ...
    def _reloadMeshFinished(self, job) -> None:
        ...
        for job_result_node in job_result:
            if job_result_node.getId() == job._node.getId():
                ...
                break
        if not object_found:
            Logger.warning(f"The object with id {job_result_node.getId()} no longer exists! Keeping the old version in the scene.")
            return
        if not mesh_data:
